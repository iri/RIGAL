...
struct LOC_expr *LINK;
{
  ...
  if (des(l) == keyword)
  { /* ob'ekt neizwestnogo tipa w wyravenii */
    switch (valc(l))
    {
  ...
  do
  {
    ...
    else
    {
      ...
      if (((1L << ((long)TEMP)) & varn_desks) != 0)
      {
        ...
        { /* movet bytj operator priswaiwanija */
          if (el(let_sign))
          {                  /* esli $a:= ... */
    ...
    _L55:
                                   ...
                                   (1L << ((long)fatom)) | (1L << ((long)spec)))) != 0)
      {
        ...
        if (((1L << ((long)TEMP2)) & varn_desks) != 0)
        {
          ...
          { /* ne nado powtorno pechatatj imja perem.*/
            next(&l);
            if (l.nel == 0)
              err(699L);
            if (er)
...
ptr_ pp;
{
  ...
  { /* this branch is added 15-jul-91 */
    if (pp.cel == saveladr)
      return saveelkey;
    else
    {
...
long av, ar;
{
  ...
  if (av != 0)
  {
    ...
    if (x1.svd->name != null_ && x1.svd->name != ass_1)
    {
      ...
      while (f2.nel != 0)
      { /* poka ne konchilsja spisok peremennyh ... */
        vnum++;
